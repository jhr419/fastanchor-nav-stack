...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1)
{
  ...
  set_input_rotation_rpy(0.0, 0.0, 0.0);
}
...
void Preprocess::set_input_rotation_rpy(double roll_rad, double pitch_rad, double yaw_rad)
{
  ...
  // 这里生成的矩阵只用于原始雷达点云进入 FAST_LIO 前的坐标转换，
  // 不等同于 FAST_LIO 估计/使用的 LiDAR-IMU 外参 extrinsic_R/extrinsic_T。
...
{
  // 对每个原始雷达点执行同一个 RPY 旋转，使点云输入坐标系与 IMU 输入坐标系保持一致。
  pt.x = input_rotation[0] * x + input_rotation[1] * y + input_rotation[2] * z;
  pt.y = input_rotation[3] * x + input_rotation[4] * y + input_rotation[5] * z;
  pt.z = input_rotation[6] * x + input_rotation[7] * y + input_rotation[8] * z;
}

// void Preprocess::rotate_input(double x, double y, double z, PointType &pt) const
// {
//   cout << "调用自定义雷达旋转函数" << endl;
//   // 对每个原始雷达点执行同一个 RPY 旋转，使点云输入坐标系与 IMU 输入坐标系保持一致。
//   double pitch_rad =0.485596;

//   pt.x = 1 * x + 0 * y + input_rotation[2] * z;
//   pt.y = 0 * x + cos(pitch_rad) * y + (-sin(pitch_rad)) * z;
//   pt.z = 0 * x + sin(pitch_rad) * y + cos(pitch_rad) * z;
// }
...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        // Livox CustomMsg 点云输入坐标转换，先旋转坐标，再参与后续特征提取。
        rotate_input(msg->points[i].x, msg->points[i].y, msg->points[i].z, pl_full[i]);
  ...
  else
  {
    for (uint i = 1; i < plsize; i++)
    {
          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        {
          // Livox CustomMsg 普通面点路径，同样先做输入坐标转换。
          rotate_input(msg->points[i].x, msg->points[i].y, msg->points[i].z, pl_full[i]);
          pl_full[i].intensity = msg->points[i].reflectivity;
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    // MID360 PointCloud2 路径的点云输入坐标转换。
    rotate_input(pl_orig.points[i].x, pl_orig.points[i].y, pl_orig.points[i].z, added_pt);
    added_pt.intensity = pl_orig.points[i].intensity;
