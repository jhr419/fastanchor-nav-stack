...
void LIONode::publishPathOnce(const std::string& map_path) {
    std_msgs::msg::String msg;
    msg.data = map_path;
    map_path_pub_->publish(msg);
    RCLCPP_INFO_STREAM(rclcpp::get_logger("lio"), "[Rolocation Mode] path_string: " << map_path);
}

void LIONode::publish_imu_odometry(State state, double lidar_end_time_) {
    ...
    odom_imu.header.stamp = get_ros_time(lidar_end_time_);
    ...
    //Path
    static nav_msgs::msg::Path odom_path;
...
void LIONode::publish_body_odometry(State state, double lidar_end_time_) {
    if (lidar_end_time_ < 0) {
        lidar_end_time_ = lidar_end_time;
    }
    nav_msgs::msg::Odometry odom_body;
    odom_body.header.stamp = get_ros_time(lidar_end_time_);
    ...
    lidar_T_body.block<3, 3>(0, 0) = lidar_R_vehicle;
    lidar_T_body.block<3, 1>(0, 3) = lidar_t_vehicle;
    // 对变换本身操作使用右乘
...
void LIONode::publishGlobalMap() {
    ...
    if (!down_effect_cloud_lidar->points.empty() && last_time != lidar_end_time) {
        ...
        if(log_save_enable && pub_time_log_enable) {
            ...
            ofstream outfile;
            outfile.open(string(PACKAGE_ROOT_DIR)+"/pub_time_log.txt", ios::app);
            outfile << duration_ms.count() << " " << duration_ms1.count() << " " << duration_ms2.count() <<  endl;
            outfile.close();
        }
...
void LIONode::publishIKDTree() {
    if (ikdtree.Root_Node != nullptr && ikdtree_pub_enable) {
        ...
        if(duration_ms.count() > 10){
            ...
            cerr << "IKDTree pub cost more than 10ms! stop publish!" << endl;
        }
    }
}
