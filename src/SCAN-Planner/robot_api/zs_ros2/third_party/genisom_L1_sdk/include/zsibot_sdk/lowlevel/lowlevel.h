...
extern "C"
{
    ...
    typedef struct bms_data_def
    {
        ...
        float power;   // 0～100%
        float temp;    // 0.001摄氏度
        int work_state; // 高8位，EM_IDLE=0, EM_CHARGE=1, EM_DISCHARGE=2, EM_SLEEP=3, EM_PROTECT=4, EM_SHUTDOWN=5   低8位，bit0-charge mos tontrol     bit1-discharge mos control   bit2- charge mos state  bit3-discharge nos state    bit7-Charger insertion detection     other bits reserve
    }__attribute__((packed)) bms_data_t,
        *bms_data_p;

    static inline bms_data_p create_bms_shm(void)
    {
        const char *BMS_SHM_PATH = "/bms_shm";
        const size_t BMS_SHM_SIZE = (1024 * 1);
        int shm_fd = shm_open(BMS_SHM_PATH, O_CREAT | O_RDWR, 0666);
        if (shm_fd == -1)
        {
            perror("shm_open.");
            exit(1);
        }
        if (ftruncate(shm_fd, BMS_SHM_SIZE) == -1)
        {
            perror("ftruncate");
            exit(1);
        }
        void *shm_ptr = mmap(NULL, BMS_SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
        if (shm_ptr == MAP_FAILED)
        {
            perror("mmap");
            exit(1);
        ...
        close(shm_fd);
        return (bms_data_p)shm_ptr;
    }

    static inline void destroy_bms_shm(bms_data_p ptr)
    {
        // don't need to unlink and unmap.
    }
