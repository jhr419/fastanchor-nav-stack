 ...
 * @brief 遥控模式示例 - 使用 ROLE_REMOTE 角色控制机器狗
 *
 * 本示例演示如何通过键盘输入控制机器狗的运动和状态切换。
 * 角色为遥控器（ROLE_REMOTE），使用默认 IP 192.168.234.1。
 *
 * 按键说明：
 *   w/a/s/d - 前后左右移动
 *   q/e     - 左转/右转
...
int main()
{
    ...
    // 使用默认的机器狗IP 192.168.234.1  使用默认的发送端口 8081 , 默认的接收端口 8080,  默认的角色 ROLE_REMOTE
    ZsibotExecutor zsibot_exec;
