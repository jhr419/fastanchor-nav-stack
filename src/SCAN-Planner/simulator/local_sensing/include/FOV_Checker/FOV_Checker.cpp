...
void FOV_Checker::check_fov(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, vector<BoxPointType> &boxes){
    ...
    // printf("Environment is: (%0.3f,%0.3f,%0.3f),(%0.3f,%0.3f,%0.3f)\n",env.vertex_min[0],env.vertex_min[1],env.vertex_min[2],env.vertex_max[0],env.vertex_max[1],env.vertex_max[2]);
    ...
    for (i=start_i; i<=maxn; i++){
        ...
        if (index == 1 || index == 4){
            ...
            plane_u_min = env.vertex_min[1] - 0.5*box_length;
            plane_u_max = env.vertex_max[1] - 0.5*box_length;
        } else {
            if (index == 2 || index == 5){                            
                ...
                max_ulogN = floor(log2(max_uN));
                max_vN = ceil((env.vertex_max[2]-env.vertex_min[2])/box_length);
                plane_u_min = env.vertex_min[0] - 0.5*box_length;
                plane_u_max = env.vertex_max[0] - 0.5*box_length;             
            } else {                            
                ...
                max_uN = ceil((env.vertex_max[0]-env.vertex_min[0])/box_length);
                max_ulogN = floor(log2(max_uN));
                max_vN = ceil((env.vertex_max[1]-env.vertex_min[1])/box_length);
                plane_u_min = env.vertex_min[0] - 0.5*box_length;
                plane_u_max = env.vertex_max[0] - 0.5*box_length;                
            }
        }
        ...
        for (j = 1; j <= max_vN; j++){
            ...
            // if (j == 1) printf("---- DOWNSIDE (%0.3f,%0.3f,%0.3f),(%0.3f,%0.3f,%0.3f)\n",box_p_min[0],box_p_min[1],box_p_min[2],box_p_max[0],box_p_max[1],box_p_max[2]);
