...
private:
  ...
  std::vector<GridKey> neighbor_offsets_2d_;
  std::vector<GridKey> neighbor_offsets_3d_;
...
private:
  ...
  bool shouldAcceptGoalRequest(const Eigen::Vector3d & goal, const std::string & source)
  {
    const rclcpp::Time stamp = now();
    if (last_goal_request_.has_value() && last_goal_request_time_.nanoseconds() != 0) {
      const double xy_shift = (goal.head<2>() - last_goal_request_->head<2>()).norm();
      const double z_shift = std::abs(goal.z() - last_goal_request_->z());
      const double elapsed = (stamp - last_goal_request_time_).seconds();
      if (xy_shift < goal_replan_min_distance_ && z_shift < goal_replan_min_z_distance_ &&
        elapsed < goal_replan_min_interval_sec_)
      {
        RCLCPP_DEBUG(
          get_logger(),
          "Ignoring duplicate A* goal from %s: xy_shift=%.3fm z_shift=%.3fm elapsed=%.3fs",
  ...
  {
    if (!replan_on_start_update_ || !active_goal_.has_value() || planning_in_progress_) {
      return;
    }

    const rclcpp::Time stamp = now();
    if (last_start_replan_attempt_time_.nanoseconds() != 0 &&
      (stamp - last_start_replan_attempt_time_).seconds() < start_replan_min_interval_sec_)
    {
      return;
    }
    ...
    const double distance_to_goal = (active_goal_->head<2>() - start.head<2>()).norm();
    ...
    if (last_planned_start_.has_value()) {
      ...
      RCLCPP_INFO(
        get_logger(),
        "A* replanning from updated start: trigger=%s moved=%.3fm threshold=%.3fm",
        trigger.c_str(), moved_distance, start_replan_min_distance_);
    } else {
      RCLCPP_INFO(
        get_logger(),
        "A* replanning from updated start: trigger=%s no previous planned start",
        trigger.c_str());
    }
