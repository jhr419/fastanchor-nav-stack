...
class MapViewerRosNode(Node):
    def __init__(self) -> None:
        ...
        self.start_sub = self.create_subscription(
            PointStamped, "/start_point", self._on_start_point, latched_qos
        )
        ...
        self._latest_start_point: tuple[float, float, float] | None = None
        self._start_point_dirty = False
    ...
    def _on_start_point(self, msg: PointStamped) -> None:
        self._latest_start_point = (
            float(msg.point.x),
            float(msg.point.y),
            float(msg.point.z),
        )
        self._start_point_dirty = True
    ...
    def consume_start_point(self) -> tuple[float, float, float] | None:
        if not self._start_point_dirty or self._latest_start_point is None:
            return None
        self._start_point_dirty = False
        return self._latest_start_point
...
class MapViewerWindow(QWidget):
    ...
    def __init__(self) -> None:
        ...
        default_map_package = Path(
            os.environ.get("MAP_VIEWER_DEFAULT_PACKAGE", str(Path.home() / "maps" / "map"))
        ).expanduser()
    ...
    def _spin_ros_once(self) -> None:
        ...
        start_point = self._ros_node.consume_start_point()
        if start_point is not None:
            self._update_point_actor("start", start_point)
