...
class MapPackageManager(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("visual_republish_period_sec", 1.0)
        ...
        visual_republish_period = float(
            self.get_parameter("visual_republish_period_sec").value
        )
        self._visual_republish_timer = None
        if visual_republish_period > 0.0:
            self._visual_republish_timer = self.create_timer(
                max(visual_republish_period, 0.1), self._republish_visual_layers
            )
    ...
    def _with_current_stamp(self, msg):
        stamped = copy.deepcopy(msg)
        stamped.header.stamp = self.get_clock().now().to_msg()
        return stamped

    def _republish_visual_layers(self) -> None:
        if self._latest_occupied is not None:
            self.occupied_pub.publish(self._with_current_stamp(self._latest_occupied))
        if self._latest_preblocked is not None:
            self.preblocked_pub.publish(self._with_current_stamp(self._latest_preblocked))
        if self._latest_traversable is not None:
            self.traversable_pub.publish(self._with_current_stamp(self._latest_traversable))
        if self._latest_risk_cost is not None:
            self.risk_cost_pub.publish(self._with_current_stamp(self._latest_risk_cost))
