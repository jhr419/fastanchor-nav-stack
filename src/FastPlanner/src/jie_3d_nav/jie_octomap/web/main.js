...
let lastStartVisualPoint = null;
let lastStartVisualYaw = 0;
...
function updateStartVisualFromPointStamped(msg) {
  if (!msg || !msg.point) {
    return;
  }
  const point = {
    x: Number(msg.point.x || 0),
    y: Number(msg.point.y || 0),
    z: Number(msg.point.z || 0),
  };
  const robotPose = resolveRobotPose();
  let yaw = robotPose ? robotPose.yaw : lastStartVisualYaw;
  if (!robotPose && lastStartVisualPoint) {
    yaw = computeYawFromPoints(lastStartVisualPoint, point, lastStartVisualYaw);
  }
  lastStartVisualPoint = point;
  lastStartVisualYaw = yaw;
  setPointVisual("start", point, yaw);
}
...
function connectRosbridge(url = wsInput.value.trim(), manual = false) {
  ...
  ros.on("connection", () => {
    ...
    startTopic.subscribe(updateStartVisualFromPointStamped);
